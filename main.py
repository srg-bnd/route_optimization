...
class RouteOptimization:
    ...
    def calculate(
        ...
        for start_name, end_name, priorities in requests:
            ...
            for crit, idx in weights.items():
                route, total = self.dijkstra(roads, start_id, end_id, idx)
    ...
    def build_road_index(
            self,
            graph # Граф дорог для построения индекса
        ):
        """
    ...
    def dijkstra(
        self, 
        graph: dict, # Граф дорог
        start: int, # ID начального города
        end: int, # ID конечного города
        weight_idx: int # Индекс веса (0 - длина, 1 - время, 2 - стоимость)
    ) -> tuple[list[int], int]:
        """
        Поиск кратчайшего пути с использованием алгоритма Дейкстры.
        
        Возвращает кортеж:
        - маршрут в виде списка ID городов
        - суммарный вес маршрута по выбранному критерию
        """
        pq = [(0, start, [])]
        visited = set()
